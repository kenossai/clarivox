...
@section('content')
    <div style="margin-bottom:24px;padding-bottom:20px;border-bottom:1px solid rgba(240,234,231,.4)">
        <span class="badge" style="font-size:12px">Tag</span>
        <h1 style="font-family:'Playfair Display',serif;font-size:36px;font-weight:700;color:#2A1818;margin:8px 0">
            #{{ $tag->name }}</h1>
    </div>
    ...
    @if ($articles->isNotEmpty())
        ...
        @endif
    @else
        <p style="font-size:12px;color:#9A8A84;padding:40px 0;text-align:center">No articles tagged with
            "{{ $tag->name }}" yet.</p>
    @endif
@endsection
